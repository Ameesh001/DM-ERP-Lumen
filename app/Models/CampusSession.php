...
class CampusSession extends Model
{
    // use HasFactory;

    protected $table = 'campus_session';
    ...
    public function rules($request, $method = null)
    {
        ...
        $rules = match ($method) {
            'POST' => [
                ...
                'session_id' => [
                    'required', Rule::unique(Constant::Tables['campus_session'], 'session_id')->where(function ($query) use ($request) {
            ...
            'PUT' => [
                ...
                'session_id' => [
                    'required', Rule::unique(Constant::Tables['campus_session'], 'session_id')->where(function ($query) use ($request) {
                        $query->where('is_enable', '<>', '2');
                        $query->where('campus_id', '=', $request->campus_id)->where('id', '<>', $request->id);
                    }),
                    'integer'
                ],
                // 'countries_id' => 'required|integer',
    ...
    public function messages($request, $method = null)
    {
        ...
        $commonMessages = [
            ...
            'session_id.unique' => [
                "code" => 10102,
                "message" => "Please provide unique Session."
            ],

//            'campus_name.min' => [
//                "string" => [
//                    "code" => 10104,
//                    "message" => "The CampusSession name must be at least :min characters."
//                ]
//            ],
//            'campus_name.max' => [
//                "string" => [
//                    "code" => 10105,
//                    "message" => "The CampusSession name may not be greater than :max characters."
//                ]
//            ],
            'campus_id.required' => [
                "code" => 10101,
