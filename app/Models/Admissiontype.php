...
class Admissiontype extends Model
{
    ...
    protected $table = 'admission_type';
    ...
    public function rules($request, $method = null)
    {
        ...
        $rules = match ($method) {
            'POST' => [
                'admission_type' => [
                    'required', Rule::unique('admission_type', 'admission_type')->where(function ($query) use ($request) {
                        $query->where('is_enable', '<>', '2');
                    }),
                    'min:3', 'max:100','regex:/^[\pL\s\-]+$/u'
                ],
               'admission_desc' => ['required','min:3', 'max:100'],
            ],
            'PUT' => [  
                'id' => 'required|integer',
                'admission_type' => [
    ...
    public function messages($request, $method = null)
    {
        ...
        $commonMessages = [
            ...
            'admission_type.unique' => [
                "code" => 10102,
                "message" => "Please provide unique Admission Type. "
            ],
            'admission_type.min' => [
                "string" => [
                    "code" => 10104,
                    "message" => "The Admission Type name must be at least :min characters."
                ]
            ],
            'admission_type.max' => [
                "string" => [
                    "code" => 10105,
                    "message" => "The Admission Type may not be greater than :max characters."
                ]
            ],
            'admission_desc.required' => [
                "code" => 10101,
                "message" => "Please provide Admission Description."
            ],
            'admission_desc.min' => [
                "string" => [
                    "code" => 10104,
                    "message" => "The Admission Description must be at least :min characters."
                ]
            ],
            'admission_desc.max' => [
                "string" => [
                    "code" => 10105,
                    "message" => "The Admission Description may not be greater than :max characters."
                ]
            ],
        ...
        $idMessages = [
            'id.required' => [
                "code" => 10107,
                "message" => "Please provide Admission Type id."
            ],
            'id.integer' => [
                "code" => 10106,
