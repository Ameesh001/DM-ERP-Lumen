 ...
 * 
 * This is a AssessmentCategory API controller
 *
 */
...
class AssessmentCategoryApi extends Controller
{
    ...
    {
        $this->mdlName = new AssessmentCategory();
    }
    ...
    public function add(Request $request)
    {
        ...
        {
            DB::beginTransaction();
            $obj = AssessmentCategory::create($request->all());
            DB::commit();
            $data = [ 'id' => $obj->id ];
            $response = Utilities::buildSuccessResponse(10000, "Assessment Category successfully created.", $data);
        }
        catch(\Exception $e)
        {
            DB::rollback();
           
            $response = Utilities::buildBaseResponse(10003, $e."Transaction Failed Assessment Category. ");
        }
    ...
    public function update(Request $request)
    {
        ...
        $Record = AssessmentCategory::find($request->id);
        ...
        if (! $Record) {
            ...
            if ($obj) {
                $data = [ 'id' => $Record->id ];
                $response = Utilities::buildSuccessResponse(10001, "Assessment Category successfully updated.", $data);
            }
        }
    ...
    /**
     * Activate/De-Activate AssessmentCategory.
     *
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function patchDisableEnable(Request $request)
    {
        ...
        $result_set = AssessmentCategory::find($request->id);
        $status = 200;
        $response = [];
        if (!$result_set) {
            $status = 404;
            $response = Utilities::buildBaseResponse(10003, "AssessmentCategory not found.");
        } else {    
            $obj = $result_set->update($request->all());
            if ($obj) {
                $data = ['id' => $result_set->id ];
                $actMsg = $request->is_enable == 1 ? "activated" : "de-activated";
                $response = Utilities::buildSuccessResponse(10001, "AssessmentCategory successfully $actMsg.", $data);
            }
        }
        return response()->json($response, $status);
    }
    ...
    /**
     * Delete AssessmentCategory.
     *
     * @param $id 'ID' of AssessmentCategory to delete. (required)
     *            
     * @return \Illuminate\Http\JsonResponse
     */
    public function delete($id, Request $request)
    {
        ...
        if (!$result_set) {
            $status = 404;
            $response = Utilities::buildBaseResponse(10003, "Assessment Category not found.");
        } else {    
            $obj = $result_set->update($request->all()); 
            if ($obj) {
                $response = Utilities::buildBaseResponse(10006, "Assessment Category successfully deleted.");
            }
        }
        return response()->json($response, $status);
    }
     ...
     *
     * @param $id 'ID' of AssessmentCategory to return (required)
     *            
     * @return \Illuminate\Http\JsonResponse
     */
    public function getOne($id, Request $request)
    {
        ...
        $result_set = AssessmentCategory::where('id', $request->id)->first();
        $status = 200;
        $response = [];
        if (!$result_set) {
            $status = 404;
            ...
            $response = Utilities::buildSuccessResponse(10005, "Assessment Category Single Data.", $dataResult);
        }
        return response()->json($response, $status);
    }

    /**
     * Fetch list of AssessmentCategory by searching with optional filters..
     *
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getAll(Request $request, $id = null)
    {
        ...
        if($request->assessment_category_name) {   
            $whereData[] = ['assessment_category_name', 'LIKE', "%{$request->assessment_category_name}%"];
        }
        ...
        $total_record_obj = AssessmentCategory::where($whereData)->active();
        ...
        $data_set_obj = AssessmentCategory::where($whereData)
            ->active()
        ...
        $response = Utilities::buildSuccessResponse(10004, "Assessment Category List.", $data_result);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getAssessmentCategory(Request $request, $id = null)
    {
        if($request->organization_id) {   
            $whereData[] = ['organization_id', $request->organization_id];
        }
